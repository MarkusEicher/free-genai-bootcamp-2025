...
def test_create_activity_without_group(db_session: Session):
    ...
    assert exc_info.value.detail["code"] == "EMPTY_GROUP_IDS"
    assert "must be specified" in exc_info.value.detail["message"]
...
def test_vocabulary_group_count(db_session: Session, test_setup):
    """Test counting vocabularies through groups."""
    activity = activity_service.create_with_validation(
        db_session,
        obj_in=ActivityCreate(
            type="flashcard",
